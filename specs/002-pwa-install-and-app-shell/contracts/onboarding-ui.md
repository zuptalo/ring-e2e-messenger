...
|---|---|---|---|
| 1 | `isStandalone === true` | **App shell** (the real app) | The only path to normal functionality. A brief (~1–2 s) branded launch splash (logo + version, fade in/out) overlays the shell on each standalone launch, then auto-dismisses. On the first standalone launch, run the onboarding-final step (permission/skip). |
